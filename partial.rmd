...
outliers2 <- county_data %>%
  left_join(police_killings, by = "county_id") %>%
  select(avg_pov, avg_urate, city, state)%>%
  arrange (desc(avg_pov))%>%
  head(5)

outliers3 <- county_data %>%
  left_join(police_killings, by = "county_id") %>%
  select(prop_n_deaths, avg_pov, avg_urate, city, state)%>%
...
Highest death per thousand: Fort Pierce in FL, with average poverty rate of 54.2.
